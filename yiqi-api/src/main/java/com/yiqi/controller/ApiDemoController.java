...
public class ApiDemoController extends BasicController {
    ...
    public Result<UserEntity> userInfo1(@ApiIgnore @RequestAttribute(AuthorizationInterceptor.USER_KEY) Long userId) {
    ...
    public Result<List<UserEntity>> userInfo() {
    ...
    public Result<UserEntity> selectUserInfoById(@PathVariable("id") Long id) {
    ...
    public Result<List<UserEntity>> selectUserInfoByCondition1(String mobile, String username) {
        HashMap<String, Object> map = new HashMap<>(2);
        map.put("mobile", mobile);
        map.put("username", username);

        List<UserEntity> userEntityList = userService.selectByMap(map);
        if(userEntityList == null || userEntityList.isEmpty()) {
            return Result.error(CodeMsg.NOT_FIND_DATA, Collections.emptyList());
        }
        return Result.success(userEntityList);
    ...
    public Result<List<UserEntity>> selectUserInfoByCondition2(String password) {
    ...
    public Result updateBatchUser() {
        ...
        user1.setUsername("aba");
        UserEntity user2 = list.get(1);
        user2.setUsername("hdfh");
